...
1. 用户A向用户B发送消息
2. **自动创建反向会话**：如果B->A的会话不存在，自动为B创建反向会话（详见6.5节）
3. **判断是否需要推送通知**：
...
当A向B发送消息时，系统自动检查并创建反向会话：

```go
// 在消息处理逻辑中（server.go 和 kafka_server.go）
func ensureReverseSession(sendId, receiveId string, sessionId string) {
    // 检查反向会话是否存在
    var reverseSession model.Session
    if err := dao.GormDB.Where("send_id = ? AND receive_id = ?", 
        receiveId, sendId).First(&reverseSession).Error; err != nil {
        if errors.Is(err, gorm.ErrRecordNotFound) {
            // 反向会话不存在，自动创建
            createReq := request.CreateSessionRequest{
                SendId:    receiveId,  // B作为发送者
                ReceiveId: sendId,      // A作为接收者
            }
            _, reverseSessionId, ret := SessionService.CreateSession(createReq)
            if ret == 0 {
                zlog.Info(fmt.Sprintf("自动创建反向会话: %s->%s, session_id=%s", 
                    receiveId, sendId, reverseSessionId))
                
                // 更新反向会话的最后消息
                dao.GormDB.Model(&model.Session{}).
                    Where("uuid = ?", reverseSessionId).
                ...
                // 清除接收者的会话列表缓存
                myredis.DelKeysWithPattern("session_list_" + receiveId)
            } else {
                // 创建会话失败，通过通知系统告知用户（方案三）
                zlog.Warn(fmt.Sprintf("自动创建反向会话失败: %s->%s", receiveId, sendId))
            }
        }
    } else {
        // 反向会话已存在，更新最后消息
        dao.GormDB.Model(&model.Session{}).
            Where("uuid = ?", reverseSession.Uuid).
            Updates(map[string]interface{}{
                "last_message":   message.Content,
                "last_message_at": time.Now(),
            })
...
在 `internal/service/chat/server.go` 和 `internal/service/chat/kafka_server.go` 的消息处理逻辑中，创建消息后调用：
...
if message.ReceiveId[0] == 'U' { // 发送给User
    ...
    // 自动创建反向会话
    ensureReverseSession(message.SendId, message.ReceiveId, message.SessionId)
...
如果自动创建反向会话失败（如数据库错误、用户被禁用等），通过通知系统告知用户：
...
if ret != 0 {
    ...
    CreateNotification(
        ...
        "新消息",
        fmt.Sprintf("%s 向您发送了一条消息", sendName),
        messageId,
        "message",
        sendId,
        sendName,
        sendAvatar,
        fmt.Sprintf(`{"session_id": "%s", "sender_id": "%s", "need_create_session": true}`, 
            sessionId, sendId),
    )
}
```

这样即使创建会话失败，用户也能通过通知得知有新消息，并可以手动创建会话。
